...
def lpips(input0, input1, model='net-lin', net='alex', version=0.1, model_dir=None):
    """
    ...
    input0_name_, input1_name_ = 'in0:0', 'in1:0'


    default_graph = tf.get_default_graph()
    producer_version = default_graph.graph_def_versions.producer

    cache_dir = model_dir if model_dir else os.path.expanduser('~/.lpips')
    ...
    with open(os.path.join(cache_dir, pb_fname), 'rb') as f:
        ...
        try:
            ...
            try:
                _ = tf.import_graph_def(graph_def,
                                    input_map={input0_name_: input0, input1_name_: input1})
            except ValueError as e:
                _ = tf.import_graph_def(graph_def)
