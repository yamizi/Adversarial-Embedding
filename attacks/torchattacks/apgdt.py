...
class APGDT(BaseAttack):
    ...
    def __init__(self, model, norm='Linf', eps=8/255, steps=100, n_restarts=1, 
                 seed=0, eot_iter=1, rho=.75, verbose=False, n_classes=10, min_pixel_value=0, max_pixel_value=1,
             threshold: float = 0.5, experiment=None, data_size=None, random_weight=0.01, random_noise="fading",
             strategy="binary"):

        super(APGDT, self).__init__("APGDT", model,steps=steps, min_pixel_value=min_pixel_value, max_pixel_value=max_pixel_value,
    ...
    def attack_single_run(self, x_in, y_in):
        ...
        y = y_in.clone() if len(y_in.shape) == 2 else y_in.clone().unsqueeze(0)
        ...
        for i in range(self.steps):
            ...
            self.plot_metrics(None, i, torch.norm(nn.Flatten()(grad_best), p=1, dim=1).mean().item(), loss_best.item(), output, y)
    ...
    def perturb(self, x_in, y_in, best_loss=False, cheap=True):
        ...
        adv = x.clone()
        success_data = success_strategy(y, self.model(x), data_size=self.data_size, strategy=self.strategy)
        acc = torch.tensor(success_data,device=self.device)
