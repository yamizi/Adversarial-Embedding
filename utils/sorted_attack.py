...
class SATA(ProjectedGradientDescent):
    ...
    @staticmethod
    def embed_message(model, x, msg,epsilon=1., num_random_init=10, max_iter=100, class_density=0.7, groups_only=False,num_classes=0):
        num_classes = num_classes if num_classes>0 else model.output_shape[1]
        ...
        adv_x = crafter.generate(x,groups, threshold, nb_classes=num_classes)

        return adv_x, SATA.rate_best

    @staticmethod 
    def craft(model, x, order, epsilon=1., num_random_init=10, max_iter=100):
        ...
        adv_x = crafter.generate(x,order, threshold)
        return adv_x
    ...
    def generate(self, x_all, order, threshold=0.1, nb_classes = 10):
        ...
        for i in range(SATA.num_cover_init):
            logger.info('Random inputs pick')
